...
以下以我在 bitFlyer 主導的「核心 App 由 UIKit 遷移至 SwiftUI」專案為例來進行說明：

#### a. 給 RD 看的版本：架構、技術選型、資料流、技術難題與解法

**背景與技術選型**

由於 bitFlyer 的核心交易 App 原本是以 UIKit 為主的舊有架構，所以累積了多年的畫面與商業邏輯之間的高度耦合，因此每次新增功能或修改交易流程時，都要花大量時間確認有沒有動到不相關的畫面、有沒有引入 race condition等問題。
由於我是 iOS 架構的主要負責人，我主導將核心產品從 UIKit 遷移到 SwiftUI，並同步導入 Swift 6 的 Swift Concurrency（async/await、actor、Sendable 等檢查）取代舊有以 GCD、completion handler 為主的非同步寫法，讓畫面狀態與非同步資料流都能在編譯期就被型別系統把關，減少執行期才爆炸的 race condition。

**遇到的技術難題**

最大的難題是「重構期間系統必須持續穩定運作、功能不能中斷」，我具體將他們拆為兩層：

1. **UI 層**：UIKit 是命令式，透過 delegate／target-action 手動更新畫面，而 SwiftUI 是宣告式、畫面由 State 驅動。兩者在遷移過程中勢必要長期共存（透過 UIViewRepresentable / UIHostingController 互相包裝），如果沒有謹慎處理狀態的單一真實來源，很容易出現「UIKit 端改了資料，SwiftUI 端沒更新」或反過來的資料不同步問題。
2. **並行層**：啟用 Swift 6 嚴格並行檢查（strict concurrency）後，許多舊程式碼裡「其實一直存在、只是剛好沒被踩到」的資料競爭會被編譯器抓到，因此需要逐一補上正確的 Sendable 標註或改寫成 actor 來進行隔離。

**解法**

我是採取「逐個擊破」的漸進式遷移，而非一次性重寫：

- 先挑風險最低、影響範圍最小的元件（例如獨立的小型顯示元件）進行 SwiftUI 化，確認在正式環境穩定運作、沒有回歸問題後，才把同樣的流程套用到下一個元件，逐步擴大到核心交易流程這類高風險模組。
- 每個元件遷移前後都建立對應的單元測試／畫面驗證，確保「邏輯轉譯成 SwiftUI／Swift Concurrency 寫法後，行為與原本的 UIKit 版本完全一致」，而不是憑感覺覺得「看起來一樣」。
- 針對 Swift 6 並行檢查，採取「一個模組一個模組」逐步開啟嚴格模式，而不是整個專案一次打開，讓每次要修的 Sendable 問題數量可控，也方便追蹤是哪次改動引入的。

最終這套做法讓整個核心產品完成架構遷移，同時大幅提升系統穩定性與可擴充性，後續新功能開發也能建立在更安全、更好維護的架構之上。

#### b. 給 PM 看的版本：解決什麼問題、對使用者的價值、時程與取捨

**解決什麼問題**

App 的舊架構讓每一次修改都伴隨著「會不會不小心弄壞其他畫面」的風險，導致開發速度隨著功能越疊越多而越來越慢，QA 與上線前的回歸測試成本也持續攀升。這個專案的目標是把 App 地基重新整頓一遍，讓後續每一次交易流程優化、每一個新功能上線都能更快、更穩。

**對使用者的價值**

- **更穩定**：交易所 App 最怕的就是關鍵時刻（例如行情劇烈波動、使用者急著下單）當機或畫面異常，架構穩定性直接對應到使用者對交易安全感的信任。
- **更順暢的下單體驗**：這次架構遷移也同步搭配核心交易流程與下單路徑的重新設計與簡化，實際降低了使用者操作阻力，對轉換率與活躍度都有正向幫助。
- **後續功能上線更快**：架構打好之後，等於是把「地基」換成更穩固、更容易擴充的版本，之後推出新功能或調整交易邏輯，開發與驗證的時間都能明顯縮短。

**時程與取捨**

由於一個交易所產品沒有那種「暫停服務三個月專心重寫」這個選項，因此 App 必須邊維運、邊上新功能、邊完成架構遷移。因此我們刻意選擇「漸進式遷移」而非「一次性重寫」：

- **取捨**：漸進式做法讓整體遷移的總時程拉得比較長（因為每個元件都要分階段驗證），但換來的是每一個階段結束後，App 都仍然是「可以正常上架、可以正常交易」的穩定狀態，不會有某幾個月因為重寫而功能停滯或風險暴增。
- 對業務端而言，這代表「架構升級」與「日常功能迭代」可以並行進行，不需要在兩者之間二選一，是用時間換取風險可控與交付穩定性的決策。


### 2. 請舉一個實際發生過的例子，展現你如何把需求分配給別的 RD 來實作

同樣以上述「UIKit 遷移至 SwiftUI」專案為例：擔任 Team Leader 期間，我是這個專案基礎架構的主要負責人，因為架構設計的好壞，直接決定了 App 長期的穩定性，也決定了後續整個團隊要怎麼分工、每個人負責的邊界在哪裡。

實際的分配方式如下：

1. **拆解需求為 Sprint 與小型 Task**：收到「把核心產品遷移到 SwiftUI」這個大方向的任務後，我不會直接把整個專案丟給團隊，而是先規劃 Sprint，把它拆解成一個個範圍明確的小型 Task（例如：非同步網路層改寫為 Swift Concurrency、共用 UI 元件庫遷移、核心交易畫面遷移、既有測試補強等）。這樣做的目的是確保進度不會因為某一個困難的子任務卡關，就拖累整體時程——其他 Task 仍然可以持續往前推進。
2. **依 RD 專長分配任務**：針對拆解出來的每個 Task，我會評估目前團隊中哪一位 RD 在該領域最擅長（例如對 Swift Concurrency 熟悉的成員，優先分配非同步網路層改寫；對動畫與互動細節掌握度高的成員，優先分配需要大量調整互動體驗的核心交易畫面元件），並優先把該 Task 指派給最適合的人，讓每個人都能發揮專長、同時降低任務執行風險。
3. **定期同步與知識分享**：分配出去之後，我會透過固定的分享會，讓負責不同 Task 的 RD 互相同步進度與踩過的坑，避免同樣的問題在不同模組被重複踩雷，也讓整個團隊對新架構、新技術的掌握度一起提升，而不是只有負責那個 Task 的人才懂。

我想這套「先拆解、再依專長分配、再同步知識」的方式，讓一個原本高風險、影響範圍極大的架構遷移專案，能夠在維持系統穩定運作的前提下，逐步且可控地完成交付。
